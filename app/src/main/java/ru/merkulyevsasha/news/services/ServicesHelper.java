...
import ru.merkulyevsasha.news.presentation.MainActivity;

public class ServicesHelper {

    private static String TAG = ServicesHelper.class.getSimpleName();

    private static double BATTERY_GOOD_PERCENT = 0.55F;

    public static final String ALARM_ACTION_NAME = "ru.merkulyevsasha.news.START_SERVICE";

    private static final int ALARM_AFTER_MINUTES = 60;

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public static void registerJobService(Context context){
        Log.d(TAG, "registerJobService");
        ...
        try {
            ...
            if (result == JobScheduler.RESULT_SUCCESS) {
                Log.d(TAG, "registerJobService done");
            }
        } catch(Exception e){
    ...
    public static void registerAlarmNewsService(Context context){
        Log.d(TAG, "registerAlarmNewsService");
    ...
    public static void setNotification(Context context, String news){
...
// Adds the Intent that starts the Activity to the top of the stack
    ...
    public static boolean isBatteryGood(Context context){
        try {
            ...
            Log.d(TAG, "isBatteryGood: " + e.getMessage());
        }
        return false;
    }
